...
void CCapsuleCollider::finaltick()
{
    ...
    Matrix matPhysXScale = XMMatrixScaling(scale.x, scale.y, scale.z);
    ...
    GamePlayStatic::DrawDebugCapsule(matPhysXScale * matCenterTrans * matWorldScaleInv * Transform()->GetWorldMat(), m_Radius,
                                     m_Height / 2.f - m_Radius, m_Direction, color, true);
}
