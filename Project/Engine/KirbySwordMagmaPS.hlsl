...
PS_OUT_DEFERRED main(PS_IN input)
{
    ...
    float metallic = g_btex_1 ? MRATex.Sample(g_LinearWrapSampler, input.vUV0).r * 0.4f : MtrlMetallic;
    float roughness = g_btex_1 ? MRATex.Sample(g_LinearWrapSampler, input.vUV0).g : MtrlRoughness;
    float ao = g_btex_1 ? MRATex.Sample(g_LinearWrapSampler, input.vUV0).b : 1.f;
    if (ao >= 1.f)
    {
    ...
    output.vNormal = float4(g_btex_5 ? NormalMapping(input, NormalTex, input.vUV0, g_LinearWrapSampler, InvertNormalMapY) : normalize(input.vNormalWorld), 1.f);
