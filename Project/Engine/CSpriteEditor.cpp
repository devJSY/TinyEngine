...
void CSpriteEditor::DrawViewprot()
{
    ...
    if (is_hovered && !adding_line && ImGui::IsMouseClicked(ImGuiMouseButton_Left))
    {
        ...
        rect.Min = mouse_pos_in_canvas;
        rect.Max = mouse_pos_in_canvas;
    ...
    if (adding_line)
    {
        Rects.back().Max = mouse_pos_in_canvas;
    ...
    for (int n = 0; n < Rects.Size; n++)
        draw_list->AddRect(ImVec2(origin.x + Rects[n].Min.x, origin.y + Rects[n].Min.y),
                           ImVec2(origin.x + Rects[n].Max.x, origin.y + Rects[n].Max.y), IM_COL32(255, 0, 0, 255));
