...
void CFBXLoader::GetUV(FbxMesh* _pMesh, tContainer* _pContainer, int _iIdx, int _iUVIndex, int _iVtxOrder)
{
    for (int ElementCount = 0; ElementCount < 4; ElementCount++)
    {
        ...
        case 0: {
            ...
            _pContainer->vecUV0[_iIdx].y = 1.f - (float)lUVValue.mData[1]; // fbx uv 좌표계는 좌하단이 0,0
        }
        break;
        case 1: {
            _pContainer->vecUV1[_iIdx].x = (float)lUVValue.mData[0];
            _pContainer->vecUV1[_iIdx].y = 1.f - (float)lUVValue.mData[1]; // fbx uv 좌표계는 좌하단이 0,0
        }
        break;
        case 2: {
            _pContainer->vecUV2[_iIdx].x = (float)lUVValue.mData[0];
            _pContainer->vecUV2[_iIdx].y = 1.f - (float)lUVValue.mData[1]; // fbx uv 좌표계는 좌하단이 0,0
        }
        break;
        case 3: {
            _pContainer->vecUV3[_iIdx].x = (float)lUVValue.mData[0];
            _pContainer->vecUV3[_iIdx].y = 1.f - (float)lUVValue.mData[1]; // fbx uv 좌표계는 좌하단이 0,0
        }
        break;
        }
    }
