...
void CBlueprintEditor::init()
{
    ...
    // m_HeaderBackground = LoadTexture(string(path + "BlueprintBackground.png").c_str());
    // m_SaveIcon = LoadTexture(string(path + "ic_restore_white_24dp.png").c_str());
    // m_RestoreIcon = LoadTexture(string(path + "ic_save_white_24dp.png").c_str());
...
void CBlueprintEditor::render()
{
    ...
    ed::Begin("Node editor");
    {
        ...
        for (auto& node : m_Nodes)
        {
            ...
            // const auto ImDrawList_AddRect = [](ImDrawList* drawList, const ImVec2& a, const ImVec2& b, ImU32 col,
    ...
    /*
        ...
        auto acceptPoint = [drawList, offset_radius](const bezier_subdivide_result_t& r)
        {
            ...
            drawList->AddLine(to_imvec(r.point), to_imvec(r.point + nt * offset_radius), IM_COL32(255, 0, 0,
       255), 1.0f);
        };
...
//
// ImTextureID CBlueprintEditor::LoadTexture(const char* path)
//{
...
//}
//
// ImTextureID CBlueprintEditor::CreateTexture(const void* data, int width, int height)
//{
//    return ImGui_CreateTexture(data, width, height);
//}
//
// void CBlueprintEditor::DestroyTexture(ImTextureID texture)
//{
//    ImGui_DestroyTexture(texture);
//}
//
// int CBlueprintEditor::GetTextureWidth(ImTextureID texture)
//{
//    return ImGui_GetTextureWidth(texture);
//}
//
// int CBlueprintEditor::GetTextureHeight(ImTextureID texture)
//{
...
void CBlueprintEditor::UpdateTouch()
{
...
Node* CBlueprintEditor::FindNode(ed::NodeId id)
{
    for (auto& node : m_Nodes)
        if (node.ID == id)
            return &node;
...
Link* CBlueprintEditor::FindLink(ed::LinkId id)
{
    for (auto& link : m_Links)
        if (link.ID == id)
            return &link;
...
Pin* CBlueprintEditor::FindPin(ed::PinId id)
{
    if (!id)
        return nullptr;
...
bool CBlueprintEditor::IsPinLinked(ed::PinId id)
{
    if (!id)
        return false;
...
bool CBlueprintEditor::CanCreateLink(Pin* a, Pin* b)
{
...
void CBlueprintEditor::BuildNode(Node* node)
{
    for (auto& input : node->Inputs)
    {
        input.Node = node;
...
Node* CBlueprintEditor::SpawnBranchNode()
{
...
Node* CBlueprintEditor::SpawnDoNNode()
{
...
Node* CBlueprintEditor::SpawnOutputActionNode()
{
...
Node* CBlueprintEditor::SpawnPrintStringNode()
{
...
Node* CBlueprintEditor::SpawnMessageNode()
{
...
Node* CBlueprintEditor::SpawnSetTimerNode()
{
...
Node* CBlueprintEditor::SpawnLessNode()
{
    m_Nodes.emplace_back(GetNextId(), "<", ImColor(128, 195, 248));
    m_Nodes.back().Type = NodeType::Simple;
...
Node* CBlueprintEditor::SpawnWeirdNode()
{
...
Node* CBlueprintEditor::SpawnTraceByChannelNode()
{
...
Node* CBlueprintEditor::SpawnTreeSequenceNode()
{
...
Node* CBlueprintEditor::SpawnTreeTaskNode()
{
...
Node* CBlueprintEditor::SpawnTreeTask2Node()
{
...
Node* CBlueprintEditor::SpawnComment()
{
...
Node* CBlueprintEditor::SpawnHoudiniTransformNode()
{
...
Node* CBlueprintEditor::SpawnHoudiniGroupNode()
{
...
void CBlueprintEditor::BuildNodes()
{
    for (auto& node : m_Nodes)
        BuildNode(&node);
}

ImColor CBlueprintEditor::GetIconColor(PinType type)
{
    switch (type)
    {
    default:
...
void CBlueprintEditor::DrawPinIcon(const Pin& pin, bool connected, int alpha)
{
    IconType iconType;
...
void CBlueprintEditor::ShowLeftPane(float paneWidth)
{
    /*  auto& io = ImGui::GetIO();
      ...
      for (auto& node : m_Nodes)
      {
          ...
          else
          {
              ...
              drawList->AddImage(m_SaveIcon, ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1,
  1), IM_COL32(255, 255, 255, 32));
          }
