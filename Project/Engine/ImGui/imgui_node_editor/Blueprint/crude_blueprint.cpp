...
void crude_blueprint::Pin::Save(crude_json::value& value) const
{
    ...
    if (!m_Name.empty())
        value["name"] = to_string(m_Name);  // optional, to make data readable for humans
    if (m_Link)
...
{
    value["id"] = crude_json::number(m_Id); // required
    value["name"] = to_string(GetName());     // optional, to make data readable for humans
...
uint32_t crude_blueprint::NodeRegistry::RegisterNodeType(string_view name, NodeTypeInfo::Factory factory)
{
    ...
    typeInfo.m_Id       = id;
    typeInfo.m_Name     = to_string(name);
...
void crude_blueprint::Blueprint::Save(crude_json::value& value) const
{
    ...
    for (auto& node : m_Nodes)
    {
        ...
        nodeValue["type_name"] = to_string(node->GetTypeInfo().m_Name); // optional, to make data readable for humans
...
{
    auto value = crude_json::value::load(to_string(path));
    if (!value.second)
        return false;
...
bool crude_blueprint::Blueprint::Save(string_view path) const
{
    ...
    return value.save(to_string(path), 4);
}
